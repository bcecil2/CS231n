...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        aROut, aRCache = affine_relu_forward(X,self.params['W1'],self.params['b1'])
        aOut, aCache = affine_forward(aROut, self.params['W2'],self.params['b2'])
        scores = aOut
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers-1):
            if self.normalization == "batchnorm":
                ...
                aROut,aRCache = affineLayerReluForward(aROut,self.params['W'+str(i+1)],
                                             self.params['b'+str(i+1)],
                                             self.params['gamma'+str(i+1)],
                                             self.params['beta'+str(i+1)],
                                       self.bn_params[i])
        ...
        for _,d in self.params.items():
            loss += 0.5 * self.reg * np.sum(d*d)
        
        dx,dwlast,dblast = affine_backward(dout,cache[self.num_layers-1])
        ...
        for i in range(self.num_layers-1,0,-1):
            if self.normalization == "batchnorm":
                ...
                dx,dwi,dbi,dgammai,dbetai = affineLayerReluBackward(dx,cache[i-1])
                grads['gamma'+str(i)] = dgammai
                grads['beta'+str(i)] = dbetai
            else:
